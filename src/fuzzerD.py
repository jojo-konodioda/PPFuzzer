from Modules import scenario_mutatorD
...
class Fuzzer:
    def __init__(self, nog = 16, nop = 8, nomu = 3, execution_time = 25, scenarios = None, log_dir = None, scenario_pool = None):
        ...
        self.mutator = scenario_mutatorD.Mutator(nog = self.nog, nop = self.nop, nomu = self.nomu, scenario_pool = scenario_pool)
    ...
    def run(self):
        ...
        # Mindist calculation
        time1 = time.time()
        mindist_calculator = mindist.Mindist_calculator(scenarios_for_mutation)
        print(time.time()-time1)
