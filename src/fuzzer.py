...
class Fuzzer:
    def __init__(self, nog = 16, nop = 8, nomu = 3, execution_time = 25, scenarios = None, log_dir = None, scenario_pool = None):
        ...
        self.mutator = scenario_mutator.Mutator(nog = self.nog, nop = self.nop, nomu = self.nomu, scenario_pool = scenario_pool)
    ...
    def run(self):
        ...
        # ERF calculation
        time1 = time.time()
        erf_calculator = erf.ERF_calculator(scenarios_for_mutation)
        print(time.time()-time1)
